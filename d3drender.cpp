...
void CD3DRender::DrawPolygon(float x, float y, int triangleAmount, float radius, D3DCOLOR color) { //
	if (SUCCEEDED(Begin(D3DPT_TRIANGLEFAN))) {
		D3DColor(color);
		D3DVertex2f(x, y);
		for (int i{}; i <= triangleAmount; i++) {
			D3DVertex2f(
				x + (radius * cosf(i * 6.2831855f / triangleAmount)),
				y + (radius * sinf(i * 6.2831855f / triangleAmount))
			);
		}
		End();
	}
