...
void visuals::imguiMenuSettings() const {
	...
	if (ImGui::Button(set::i().main.check ? "Status" : "Health", { 85.f, 20.f }); ImGui::IsItemClicked()) {
	...
	ImGui::SameLine();
	if (ImGui::TextUnformatted("IgnoreList"); ImGui::IsItemHovered() && !set::i().main.ignoreListStr.empty()) {
	...
	ImGui::SameLine();
	if (ImGui::TextUnformatted("BlackList"); ImGui::IsItemHovered() && !set::i().main.blackListStr.empty()) {
		imguiShowTooltip(set::i().main.blackListStr);
	}

	if (ImGui::Button("Save settings", { 313.f, 20.f }); ImGui::IsItemClicked()) {
...
void visuals::imguiMenuMisc() const {
	if (ImGui::Button(set::i().misc.menuKeyChange ? "Press any key" : set::i().misc.menuKeyName.data(), { 85.f, 20.f }); ImGui::IsItemClicked()) {
	...
	if (static bool tg; ImGui::BeginCombo("##16", fracList.data(), ImGuiComboFlags_::ImGuiComboFlags_NoArrowButton)) {
		...
		for (const auto& buf : frac) {
			...
			if (ImGui::Selectable(buf.data(), selected)) {
				fracList = buf;
			}
			if (selected) {
				ImGui::SetItemDefaultFocus();
