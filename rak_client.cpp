...
bool rakClient::hookedSend(sampFuncs::oSend orig, void* this_, BitStream* bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel) {
	...
	case PacketEnumeration::ID_AIM_SYNC:
		if (set::i().main.packetAim && dmgFuncs::i().checkActivate() && dmgFuncs::i().remotePlayerIsExist(set::i().main.dmgId)) return false;
		break;
	case PacketEnumeration::ID_PLAYER_SYNC:
		if (set::i().main.packetVehPlayerPass && dmgFuncs::i().checkActivate() && dmgFuncs::i().remotePlayerIsExist(set::i().main.dmgId)) return false;
		if (set::i().misc.invis) processInvis.template operator()<stOnFootData>();
		break;
	case PacketEnumeration::ID_PASSENGER_SYNC:
		if (set::i().main.packetVehPlayerPass && dmgFuncs::i().checkActivate() && dmgFuncs::i().remotePlayerIsExist(set::i().main.dmgId)) return false;
		if (set::i().misc.invis) processInvis.template operator()<stPassengerData>();
		break;
	case PacketEnumeration::ID_VEHICLE_SYNC:
		if (set::i().main.packetVehPlayerPass && dmgFuncs::i().checkActivate() && dmgFuncs::i().remotePlayerIsExist(set::i().main.dmgId)) return false;
		if (set::i().misc.invis) processInvis.template operator()<stInCarData>();
		break;
	case PacketEnumeration::ID_WEAPONS_UPDATE:
		if (set::i().main.packetWeapon && dmgFuncs::i().checkActivate() && dmgFuncs::i().remotePlayerIsExist(set::i().main.dmgId)) return false;
	}
