...
set::set() noexcept {
	if (const auto [res, _] { getJson() }; !res) saveSettings();
	loadSettings();
}
...
std::tuple<bool, nlohmann::json> set::getJson(const std::string& path) const noexcept {
	try {
		nlohmann::json j;
...
void set::saveJson(const nlohmann::json& j) const noexcept {
	std::ofstream{ fileName } << std::setw(4) << j;
}

void set::loadWeaponSettings(std::uint8_t gun, bool mode) noexcept {
...
void set::processWeaponSettings(nlohmann::json& j) const noexcept {
...
void set::saveMenuSettings() const noexcept {
...
void set::saveMenuVisuals() const noexcept {
	if (auto [res, j] { getJson() }; res) {
...
void set::saveMenuMisc() const noexcept {
	if (auto [res, j] { getJson() }; res) {
		auto& misc_{ j["misc"] };
...
void set::loadSettings() noexcept {
	if (const auto [res, j] { getJson() }; res) {
		...
		if (const auto keyName{ dmgFuncs::i().getKeyNameById(main.key) }; keyName.empty()) {
			main.key = 82;
		}
		else {
			main.keyName = keyName;
		}
		...
		//misc
		if (const auto keyName{ dmgFuncs::i().getKeyNameById(misc.menuKey) }; keyName.empty()) {
			misc.menuKey = 113;
		}
		else {
			misc.menuKeyName = keyName;
		}
	}
}

void set::saveSettings() const noexcept {
	nlohmann::json j;
