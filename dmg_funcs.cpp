...
void dmgFuncs::commandFunc(char* buf) {
	if (buf[0] == '\0') {
	...
	if (p == "menu") {
		...
		if (p == "key") {
			if (p2.empty()) {
				...
				if (const auto kName{ dmgFuncs::i().getKeyNameById(*value_) }; !kName.empty()) {
			...
			if (p2 == "none") {
				...
				for (const auto& buf_ : std::string_view{ buf } | std::ranges::views::split(' ') | std::ranges::to<std::vector<std::string>>()) {
					const auto& vb{ j["settings"]["frac"][buf_] };
					if (vb.empty()) continue;
					if (dmgFuncs::i().findValue(set::i().main.fracName, buf_)) continue;
					set::i().main.fracName.push_back(buf_);
...
vector3 dmgFuncs::getLocalPlayerPos(bool wInvis) const {
