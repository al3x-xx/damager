		...
		if (p == "key") {
			...
			if (p2 == "none") {
				...
				for (const auto& buf : buf_ | std::ranges::views::split(' ') | std::ranges::views::transform(tsv)) {
					...
					if (dmgFuncs::i().findValue(set::i().main.fracName, buf)) continue;
			...
			if (const auto value2{ dmgFuncs::i().strToInt(p2) }) {
				...
				if (set::i().main.settingsForServer) {
					set::i().processWeaponSettings(j["settings"]);
				}
				else {
			...
			else if (auto value2{ dmgFuncs::i().strToInt(p2) }; value2 && --*value2 >= 0 && static_cast<decltype(set::i().main.ignoreList.size())>(*value2) < set::i().main.ignoreList.size()) {
			...
			else if (auto value2{ dmgFuncs::i().strToInt(p2) }; value2 && --*value2 >= 0 && static_cast<decltype(set::i().main.blackList.size())>(*value2) < set::i().main.blackList.size()) {
...
std::uint16_t dmgFuncs::getPlayerIdForDmg() const noexcept {
...
void dmgFuncs::processDmg(std::uint16_t id) const noexcept {
	if (set::i().main.miss) {
		static bool tg;
...
bool dmgFuncs::checkSkin(std::uint16_t id) const noexcept {
