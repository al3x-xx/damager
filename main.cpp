...
void main::hookedMainloop(oMain orig) {
	if (static bool init; !init) {
		if (sampFuncs::sampInit() && sampFuncs::i().gameStateIsConnected()) {
			...
			static const auto pRakClient{ std::make_unique<rakClient>() };
			static const auto pVisuals{ std::make_unique<visuals>() };
			static const auto pWndProc{ std::make_unique<wndProc>() };
