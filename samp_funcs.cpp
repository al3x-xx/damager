...
void sampFuncs::addChatMessage(int color, const char* message) const {
	if (ver_ == sampVer::SAMP_VER_R1) {
		reinterpret_cast<void(__thiscall*)(void*, int, const char*, char*, int, std::uint32_t)>(base_ + offsets::addChatMessage[ver_])(pChat_, 8, message, nullptr, color, 0);
	}
	else {
...
void sampFuncs::unregisterChatCommand(cmdProc commandFunc) {
