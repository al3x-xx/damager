...
sampFuncs& sampFuncs::i() noexcept {
	static sampFuncs instance;
	return instance;
}

std::tuple<std::uint32_t, sampFuncs::sampVer> sampFuncs::getSampBaseAndVer() noexcept {
	if (const auto base{ reinterpret_cast<std::uint32_t>(GetModuleHandleA("samp.dll")) }) {
		...
		case 0x31DF13: return { base, sampVer::r1 };
		case 0xCC4D0: return { base, sampVer::r3_1 };
		}
	}
	return { 0, sampVer::unk };
}

bool sampFuncs::sampInit() noexcept {
...
void sampFuncs::addChatMessage(int color, const char* message) const {
	if (ver_ == sampVer::r1) return reinterpret_cast<void(__thiscall*)(void*, int, const char*, char*, int, std::uint32_t)>(base_ + offsets::addChatMessage[ver_])(pChat_, 8, message, nullptr, color, 0);
...
void sampFuncs::disableNameTags(bool state) const noexcept {
...
stSAMPPed* sampFuncs::getLocalSampPed() const noexcept {
...
stSAMPPed* sampFuncs::getRemotePlayerSampPed(std::uint16_t id) const noexcept {
